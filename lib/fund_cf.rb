module FundCf
  ...
  def cf_fund_investment_valid?(other, cf_investment)
    if cf_investment
      ...
      other.amount += cf_investment.amount # temporarily add the entire amount to the original investment - this will test the account balance
      valid = other.valid? && cf_investment.valid?
      other.amount -= cf_investment.amount # subtract the overhead again
    else 
      valid = other.valid?
    end
    valid
