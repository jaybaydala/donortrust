...
module RoleRequirementSystem
  ...
  module RoleSecurityClassMethods
    ...
    #    require_role "admin", :only => :update, :unless => "current_busaccount.authorized_for_listing?(params[:id]) "
    #
    # Valid options
    #
    ...
    # This is the core of RoleRequirement.  Here is where it discerns if a busaccount can access a controller or not./
    def busaccount_authorized_for?(busaccount, params = {}, binding = self.binding)
      ...
      self.role_requirements.each{| role_requirement|
        ...
        passed = false
        roles.each { |role|
          passed = true if busaccount.has_role?(role)
        } unless (busaccount==:false || busaccount==false)
...
#    end
  ...
  protected
    # receives a :controller, :action, and :params.  Finds the given controller and runs busaccount_authorized_for? on it.
    ...
    def url_options_authenticate?(params = {})
      ...
      klass.busaccount_authorized_for?(current_busaccount, params, binding)
    end
  end
end