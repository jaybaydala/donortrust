...
describe Dt::CheckoutsController do
  ...
  before do
    @investment = Factory.build(:investment)
  ...
  it "should define CHECKOUT_STEPS as %w(payment billing confirm)" do
    controller.class::CHECKOUT_STEPS.should == %w(payment billing confirm)
  end
  ...
  describe "create action" do
    ...
    def do_request(params = {})
      ...
      params = @order.attributes.delete_if{|k,v| v.nil?}.merge(params)
  ...
  describe "update action" do
    ...
    it "should redirect to the payment step" do
      do_request
    ...
    describe "on the confirm step" do
      ...
      it "next_step should be nil" do
        do_request
        controller.send(:next_step).should be_nil
      end
      ...
      describe "do_confirm method" do
        ...
        it "should update the send_now on any stale gifts (with send_at values in the past)" do
          ...
          @gift = Factory(:gift, :send_email => true)
        ...
        it "should not update the send_now on any gifts that aren't being sent (send_email = false)" do
          now = Time.now
          Time.stub!(:now).and_return(now)
          @gift = Factory(:gift, :send_email => false)
          @cart.stub!(:gifts).and_return([@gift])
          @gift.should_receive(:send_at=).never
          do_request
        end
