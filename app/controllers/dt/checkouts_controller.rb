    ...
    if @order.complete?
      ...
      if @gift_card
        if @gift_card.balance == 0
          ...
          flash[:notice] = "Please note: Your gift card balance will expire on #{@gift_card.expiry_date.strftime("%b %e, %Y")}. If you need more time, please #{link_to "Deposit the balance", new_dt_account_deposit_path(current_user.id, :deposit => {:amount => @gift_card.balance})} into your account" if !@gift_card.expiry_date.nil?
        end
      end
    end
  end
