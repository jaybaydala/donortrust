...
class TaskCategoryTest < Test::Unit::TestCase
  ...
  def test_unique_create_category
    ...
    assert_invalid( clean_new_instance, :category, task_categories( :testone ).category )
  end
  
  def test_edit_to_empty_category
    # Should not be valid to modify an existing instance to have an empty or blank category
    assert_valid( TaskCategory.find( task_categories( :testone ).id ))
    assert_invalid( TaskCategory.find( task_categories( :testone ).id ), :category, nil, "" )
  end
  ...
  def test_edit_to_empty_description
    ...
    assert_invalid( TaskCategory.find( task_categories( :testone ).id ), :description, nil, "" )
  end
  ...
  def test_edit_to_duplicate_category
    ...
    assert_invalid( TaskCategory.find( task_categories( :testone ).id ), 
      :category, task_categories( :testtwo ).category  )
  end
  #destroy should fail if any Task (or history) using category
end
